...
def load_proteins_dataset():
    ogb_dataset = NodePropPredDataset(root=DATAPATH, name='ogbn-proteins')
...
def load_ogb_dataset(name):
    ...
    ogb_dataset = NodePropPredDataset(root=DATAPATH, name=name)
    dataset.graph = ogb_dataset.graph
    dataset.graph['edge_index'] = torch.as_tensor(dataset.graph['edge_index'])
    dataset.graph['node_feat'] = torch.as_tensor(dataset.graph['node_feat'])
...
def load_genius():
    filename = 'genius'
    dataset = NCDataset(filename)
    fulldata = scipy.io.loadmat(DATAPATH+'/genius.mat')
...
if __name__ == '__main__':
    ...
    # print('x', x.shape)
    # print('y', y.shape)
    # print('#classes', y.unique().shape[0])
